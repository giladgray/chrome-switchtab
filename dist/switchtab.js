...
(function() {
  var $tabs, ANIM_TIME, MAX_HEIGHT, closeTab, doHighlight, filterTabs, highlightTab, resetFilter, scrollTop, selectTab, template, updateLabel;

  MAX_HEIGHT = localStorage['switchtab_height'] || 400;
  ...
  scrollTop = function(amount) {
    return $tabs.animate({
      scrollTop: $tabs.scrollTop() + amount
    }, ANIM_TIME);
  };
  ...
  updateLabel = function(resize) {
    var size;
    $('#count').text(size = $('.tab.match').size()).css('background-color', (function() {
      switch (size) {
        case 0:
          return 'firebrick';
        case 1:
    ...
    if (resize) {
      return setTimeout(function() {
        return $('#switchtab').height(Math.min($tabs.height() + $tabs.offset().top, MAX_HEIGHT));
      }, ANIM_TIME);
    }
  };
  ...
  filterTabs = function(tabs, query) {
    ...
    $tabs.find('.tab').each(function(i) {
      ...
      if (regex.test($el.find('.title').text())) {
        return $el.addClass('match');
      } else {
        return $el.removeClass('match');
      }
    });
    $tabs.find('.match:first').takeClass('active');
    return updateLabel(false);
  };
  ...
  chrome.tabs.query({}, function(result) {
    var $window, tab, thisWindow, _i, _len;
    $tabs = $('#tabs');
    $window = null;
    thisWindow = -1;
    for (_i = 0, _len = result.length; _i < _len; _i++) {
      tab = result[_i];
      if (tab.windowId !== thisWindow) {
    ...
    $('#search').keyup(function(event) {
      ...
      switch (event.which) {
        case 13:
          ...
          break;
        case 38:
          return doHighlight($('.tab.match').index($('.tab.active')) - 1);
        case 40:
          return doHighlight($('.tab.match').index($('.tab.active')) + 1);
        default:
